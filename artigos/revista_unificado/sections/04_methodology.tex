...
The learned comparison comprises XGBoost \cite{chen2016xgboost}, a direct LSTM
\cite{hochreiter1997}, TimesNet, and a direct DilatedRNN. These models share
the scaled GHI sequence and site identifier;
...
partitions. For output length greater than one, the implementation uses
\texttt{multi\_output\_tree} with histogram-based training: each tree has a
shared split structure and vector-valued leaves for the forecast leads
\cite{xgboostmultioutput}. The one-month task uses a scalar-output regressor.
This multi-output facility is a later library feature, distinct from the
scalar-tree formulation in the original XGBoost paper.
The hourly extension is narrower: all official model forecasts are
