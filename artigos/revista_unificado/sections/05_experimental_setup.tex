...
For all neural models, the selection and fresh-refit procedure in
Algorithm~\ref{alg:matched-training} uses early stopping on 2023
site-normalized validation MSE. Initial training uses the adjustment
partition recorded in the saved artifacts; the selected epoch count governs
the same-seed refit on pre-2024 windows. This selection objective differs from
the primary test ranking by macro-MAE in physical units. XGBoost uses
separate initial-training and refit instances with a fixed tree budget.
Only refitted instances generate test forecasts.
