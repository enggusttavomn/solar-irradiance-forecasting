...
This appendix records the complete configuration needed to reconstruct the
matched experiments without inferring values from narrative descriptions.
...
\subsection{Complete temporal contract}
...
retained only when every target lies on or before that date.
...
\midrule
Hourly 336--72 & Adjustment & 2019-01-15--2022-12-29 & 2022-12-31 & 1,445 \\
...
\addlinespace
Daily 365--30 & Adjustment & 2020-01-01--2022-12-02 & 2022-12-31 & 1,067 \\
...
\addlinespace
Monthly 12--1 & Adjustment & 2020-01-31--2022-12-31 & 2022-12-31 & 36 \\
...
\addlinespace
Monthly 12--6 & Adjustment & 2020-01-31--2022-07-31 & 2022-12-31 & 31 \\
...
are frozen before the relevant evaluation year, while the context advances and
...
\FloatBarrier
\subsection{Complete model and optimization settings}

The common learned-model input is an array of shape
(batch, context length) containing only scaled GHI, accompanied by one integer
site identifier. The target is a continuous direct vector of the declared
output length. No learned model receives latitude, longitude, clock variables,
solar elevation, climate, cloud, precipitation, or a previously predicted
target. Table~\ref{tab:complete_hyperparameters} records the frozen model,
optimizer, seed, and ensemble settings.

\begin{table}[!t]
\centering
\caption{Complete frozen learned-model settings.}
...
\cite{chen2016xgboost,xgboostmultioutput} \\
Refit rule & Fresh initialization under the same seed and exactly the epoch
count selected on 2023; XGBoost is independently fitted with its fixed budget \\
...
Hourly daily-profile persistence repeats the last 24 observed local hours
cyclically over the output, and its annual reference preserves the same local
hour from the preceding year. For daily/monthly data, persistence repeats the
last observed aggregate over the full output. For daily/monthly data, annual
seasonal na\"ive uses the same civil day or month from the preceding year, with
29 February mapped to 28 February when needed. Daily/monthly climatology is the
fixed mean for the target calendar day or month using values before the
relevant selection or test cut; a global pre-cut mean is the fallback for a
missing calendar key. Baselines and learned forecasts receive a zero floor.
Only hourly outputs additionally receive the nonpositive-solar-elevation mask,
evaluated at minute 30 of each interval.

\FloatBarrier
\subsection{Evaluation and artifact contract}
...
selected epoch counts, training histories, and the two sets of pre-cut scales.
...
\FloatBarrier
\subsection{Geographical and meteorological provenance}

The geographical CSV retains all factory-coordinate provenance, raster-cell
row and column, cell-centre coordinates, sampling distance, class code and
...
Beck class legend & Codes, descriptions, and RGB colours; MD5
...
The climate source is the present nominal 1-km K\"oppen--Geiger product of
Beck et al. \cite{beck2018}. The meteorological artifact contains 3,660
...
\cite{nasapowerdaily}. Its declared use is post-hoc error context, with rain at
1~mm/day or more, very cloudy conditions at 80\% cloud amount or more, and the
logical union as the adverse-day flag. The manifest explicitly records that
these variables are not model inputs.
